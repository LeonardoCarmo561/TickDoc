...
import { getSectorHistorical } from '@/services'
import { HistoricalData } from '@/@types'
import { formatDatetime } from '@/utils'
import { useFetch } from '@/utils/hooks'

export function HistoricalSector(props: {
  sectorId: number
  revalidate: boolean
  onRevalidate: () => void
}) {
  const { data, error, isLoading, revalidate } = useFetch<HistoricalData[]>(
    getSectorHistorical(props.sectorId),
    props.onRevalidate,
  )
        ...
        {data &&
          data.map((historical, index) => {
            if (historical.type !== '+') {
              ...
              return (
                <div className="flex flex-col gap-1 p-2" key={index}>
                  <span className="flex-wrap break-words">Setor criado</span>
                  <span className="flex-wrap break-words">
